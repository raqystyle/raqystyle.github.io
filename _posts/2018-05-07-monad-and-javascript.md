...
keywords:   "monads, natural transformations, monad transformers, contexts"
desc:       "This article is about monads and computational contexts and operations we can perform with them"
permalink:  "monad-and-javascript"
---
...
- `Either a b = Left a | Right b` for the computation might have two outcomes. Traditionally, `Left` value is for an unfortunate or erroneous result. `Right` is for a successful outcome. Notice that this type takes two other types - `a` and `b`. As such, when you `map` over `Either`, you only exchange `b`. Therefore, if the computation failed, the error is propagated further, not `b`.
...
- [Tom Harding's Blog](http://tomharding.me/)
